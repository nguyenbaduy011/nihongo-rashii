...
import { grammars, levels } from "@/lib/drizzle/schema";
import { eq } from "drizzle-orm";
import { redirect } from "next/navigation";

export default async function DemoPage({params}: {params:{N_number: string}}) {;
  ...
  const data = await db.select().from(grammars).where(eq(grammars.level, params.N_number));
